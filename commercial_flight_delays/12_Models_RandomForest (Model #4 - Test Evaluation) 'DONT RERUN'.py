...
# MAGIC %md
# MAGIC
# MAGIC # Random Forest Model - Final Test Evaluation
# MAGIC
# MAGIC This notebook is performing the final evaluatoin of this Random Forest model on the Test dataset.
# MAGIC Within this notebook,  we are reading in the full 2015-2019 dataset, breaking off 23% (approx. 2019) as our test set for evaluation.
# MAGIC A threshold will be selected from the train set of these results to apply to the test set. 
# MAGIC
# MAGIC *PLEASE NOTE: Variable assigments reference 'cross-validation' & 'validation' because of reuse of pre-existing functions.  This is a final train and test compute.*
# MAGIC
...
df = spark.read.parquet(f"{team_blob_url}/BK/clean_5yr_eng").cache()
# df = df[df['sched_depart_date_time_UTC'] < '2015-02-01']

# model specifics

# directory for train results
pure_val_train = 'ES/RF/Model4_finaltest_train'
# directory for test results
pure_val_val = 'ES/RF/Model4_finaltest_test'

# specify number of cross_validation blocks, put None if not wanted
cross_blocks = None

# to reduce the size of the train set by ratio
train_ratio = 1

# initialize classifier
dt_model = RandomForestClassifier(labelCol='label', 
...
def random_forest_model(df_tuple, class_weight, rf_model, verbose=True):
    ...
    # retreiving results
    result_cols = ['probability', 'prediction', 'label', 'sched_depart_date_time_UTC', 'TAIL_NUM']
...
categorical_features = ['OP_UNIQUE_CARRIER', 
                        ...
                        # 'TAIL_NUM', # too granular for OHE compute
                        # 'OP_CARRIER_FL_NUM', # too granular for OHE compute
                        # ]

airport_numeric = ['Av_airport_delay',
                   'hourly_flights_origin',
                   'airport_congestion',
                   ]

airline_numeric = ['Av_carrier_delay',
                    'Prev_delay_tailnum',
                    'weekly_flights_tailnum',
                    ]

weather_numeric = ['origin_6Hr_Precipitation',
...
time_numeric = ['day_of_year']

numeric_features = airport_numeric + airline_numeric + weather_numeric + time_numeric

#features + timestamp + label + splitting column
relev_data = df.select(*categorical_features, *numeric_features, col("sched_depart_date_time_UTC"), col('TAIL_NUM'), col("DEP_DEL15").alias("label"))


# COMMAND ----------

# OHE categorical features
result, ohe_feat = ohe_features(relev_data, categorical_features)

# creating a list of features and categorical values
lbl_lst = readable_feature_list(result, categorical_features, numeric_features)
print(len(lbl_lst))

# COMMAND ----------

# creating new relevant features list
features = ohe_feat + numeric_features
...
# MAGIC %md 
# MAGIC ## Create Final Train and Test Sets
# MAGIC Using a 77% train percentage to get us close to the 2019 split. 
# MAGIC
# MAGIC *PLEASE NOTE: Variable assigments reference 'cross-validation' & 'validation' because of reuse of pre-existing functions.  This is a final train and test compute.*
...
pure_val_tuples = create_validation_blocks(assembled_df, "sched_depart_date_time_UTC", blocks=1, split=0.77)

# create cross-validations sets if desired
if cross_blocks:
    cross_val_tuples = create_validation_blocks(pure_val_tuples[0][0], "sched_depart_date_time_UTC", blocks=cross_blocks)
elif train_ratio:
    # if no cross-validation, reduce the train dataset to the reduced ratio specified 
    # using splitting function to quantity a reduced fraction of train
    resized_train = create_validation_blocks(pure_val_tuples[0][0], "sched_depart_date_time_UTC", blocks=1, split=(1-train_ratio))
    # re-creating tuple with reduced train
    cross_val_tuples = [(resized_train[0][1], pure_val_tuples[0][1])]
else:
    # otherwise carry forward with full original train and validation split
    cross_val_tuples = pure_val_tuples
...
for i, df_tuple in enumerate(cross_val_tuples):
    ...
    train_results, validation_results, feature_importances = random_forest_model(df_tuple, class_weights[i], dt_model)
...
funcs.write_parquet_to_blob(all_train_results, pure_val_train)
funcs.write_parquet_to_blob(all_val_results, pure_val_val)
...
# read in model results from parquet
team_blob_url = funcs.blob_connect()
all_train_results = spark.read.parquet(f"{team_blob_url}/{pure_val_train}")
all_val_results = spark.read.parquet(f"{team_blob_url}/{pure_val_val}")
...
# plotting results
plot_precision_recall(train_table, val_table,  xlims =(55,100), ylims=(15,40))
display(train_table)
display(val_table)
...
# MAGIC %md
# MAGIC
# MAGIC ### Reviewing Feature Importance

# COMMAND ----------

# initiate a pandas df
feature_importances = pd.DataFrame(columns = ['Fold','Index', 'Feature', 'IG'])
# iterating through each cross validation fold
for i, fold in enumerate(lst_feat_import):
    print("-"*70)
    print(f"Cross-Val #{i}")
    # iterating through each significant index and return label name and IG
    for j in fold.indices:
        print(f"{lbl_lst[j]} = {fold[int(j)]}")
        # create new row for df
        # row = [i, j, lbl_lst[j], fold[int(j)]]
        # add row to df
        # feature_importances[len(feature_importances)] = row
