...
# MAGIC %md
# MAGIC
# MAGIC # Random Forest Model - Grid-Search and Cross Validation
# MAGIC
# MAGIC This notebook is the final grid search and cross validation performed on the selected model.
# MAGIC
...
df = spark.read.parquet(f"{team_blob_url}/BK/clean_5yr_WITHOUT_2019_eng").cache()
# df = df[df['sched_depart_date_time_UTC'] < '2015-02-01']

# model specifics

# directory for train results
pure_val_train = 'ES/RF/Model3_grid_ctrain'
# directory for test results
pure_val_val = 'ES/RF/Model3_grid_cval'
...
paramGrid = ParamGridBuilder().addGrid(rf_model.numTrees, [50, 75, 100]) \
                            .addGrid(rf_model.maxDepth, [5, 10, 15]) \
                            .addGrid(rf_model.minInfoGain, [0.01, 0.001, 0.0001]) \
                            .build()
...
def random_forest_model(df_tuple, class_weight, rf_model, verbose=True):
    ...
    model_pipeline = Pipeline(stages=[rf_model])

    # initiating crossvalidator for param grid search
    ...
    # fit for cross-val
    rf_fitted = crossval.fit(dwnsmpl_train)

    # capturing the best parameters for the model
    param_grid = rf_fitted.getEstimatorParamMaps()
    avg_metrics = rf_fitted.avgMetrics

    # Print the average metric value for each param map
    for i, (param_map, avg_metric) in enumerate(zip(param_grid, avg_metrics)):
        print(f"Param Map {i + 1} - Avg Metric: {avg_metric:.4f}")
        for param, value in zip(param_map.keys(), param_map.values()):
            print(f"  {param.name}: {value}")

    ###### Commenting out because grid search doesn't have featureImportances ###########
    ...
    return train_results, validation_results, param_grid # feature_importance - commented out because grid search doesnt have feature importance
...
for i, df_tuple in enumerate(cross_val_tuples):
    ...
    train_results, validation_results, best_model_params = random_forest_model(df_tuple, class_weights[i], rf_model)
    lst_train_results.append(train_results)
    lst_val_results.append(validation_results)
    lst_best_params.append(best_model_params)
